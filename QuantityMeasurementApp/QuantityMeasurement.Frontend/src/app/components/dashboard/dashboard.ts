...
export class DashboardComponent implements OnInit {
  ...
  runOperation(): void {
    ...
    this.message = '';
    this.result = 'Running operation...';

    this.quantity.runOperation(endpoint, body).subscribe({
      next: (res: unknown) => {
  ...
  private validateForm(): string {
    if (!this.isInteger(this.value1)) {
      return 'Value 1 must be an integer.';
    }

    if ((this.operator === 'Add' || this.operator === 'Subtract' || this.operator === 'DivideByQuantity') && !this.isInteger(this.value2)) {
      return 'Value 2 must be an integer for the selected operation.';
    }

    if (this.operator === 'DivideByScalar') {
      if (!this.isInteger(this.scalar)) {
        return 'Scalar must be an integer.';
      }
